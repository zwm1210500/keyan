...
if __name__ == '__main__':
    ...
    text_list = []
    # 读取 pos
    temp = loadDocs('./结巴分词/yuliao_pos.csv')
    pos_num = len(temp)
    ...
    # 读取 unlabel
    temp = loadDocs('./结巴分词/yuliao_unlabel.csv')
    ...
    content = f.read()
    f.close()
    stop_word_list = str(content).split('\r\n')
    ...
    f = codecs.open('./test/jieba_segment.txt', 'w', encoding='utf-8')
    ...
    f = codecs.open('./test/feature_set.txt', 'w', encoding='utf-8')
