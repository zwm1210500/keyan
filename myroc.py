...
# io 预备
# 读取 pos
f1 = open("./test/yuliao_pos.csv", 'r', encoding="utf-8")
content = f1.read()
f1.close()
...
# 读取 unlabel
f2 = open("./test/yuliao_unlabel.csv", 'r', encoding="utf-8")
content = f2.read()
f2.close()
temp = content.split('\n')
...
for i in range(0, len(text_list)):
    ...
    text_list[i] = jieba.lcut( text_list[i][-1], cut_all=False )
...
def getCenter(p_vector, u_vector):
    ...
    # print('\n')
    # print(p_center.tolist())
    # p_center = norm(p_center)
    # print(p_center.tolist())

    # print('\n')
    # print(u_center.tolist())
...
p_vector = weight[0:pos_num].tolist()
u_id = list( range(1, unlabel_num + 1) )
u_vector = weight[pos_num:].tolist()
...
for i in range(0, len(RN_vector_pre)):
    ...
    # 找到距离当前文档最近的正例中心向量
    for u in u_center:
        if p_d_dist <= cos_sim(d, u):
            RN_id.append(RN_id_pre[i])
            RN_vector.append(d)
            break
# 计算余弦距离，重新确定可靠负例

print('Here is them: ', len(RN_id))
print(RN_id)
