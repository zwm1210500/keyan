...
if __name__ == '__main__':
    ...
    # io 预备
    # 处理 pos
    text_list = loadDocs("./Lpu_Input/yuliao_pos.csv")
    ...
    fw = codecs.open('./Lpu_Input/yuliao_pos.nlpresult', 'w', encoding='utf-8')
    ...
    # 处理 unlabel
    text_list = loadDocs("./Lpu_Input/yuliao_unlabel.csv")
    ...
    fw = codecs.open('./Lpu_Input/yuliao_unlabel.nlpresult', 'w', encoding='utf-8')
    for doc in text_list:
        if doc[-1] != []:
            for i in range(0, len(doc) - 1):
                fw.write(doc[i] + '|')
    ...
    # 处理 test
    text_list = loadDocs("./Lpu_Input/yuliao_test.csv")
    ...
    fw = open('./Lpu_Input/yuliao_test.nlpresult', 'w', encoding='utf-8')
    for doc in text_list:
        if doc[-1] != []:
            for i in range(0, len(doc) - 1):
                fw.write(doc[i] + '|')
